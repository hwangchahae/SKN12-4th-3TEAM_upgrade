...
def save_analysis_log(repo_url: str, file_count: int, directory_structure: str, total_time: float, session_id: str = None):
    ...
    try:
        ...
        log_content.append("GitHub 저장소 분석 결과 로그 + 필요시 역할 태깅으로 수정 (기록용) + aiohttp도입&큐관리&세마포어조정&비동기파일처리")
        log_content.append("=" * 80)
...
class GitHubRepositoryFetcher:
    ...
    async def get_all_main_files_async(self, path="", session=None):
        """비동기로 파일 목록을 가져오는 개선된 함수"""
        files = []
        
        # aiohttp 세션이 없으면 새로 생성
        if session is None:
            async with aiohttp.ClientSession() as session:
                return await self.get_all_main_files_async(path, session)
        ...
        try:
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    print(f"[ERROR] GitHub API 오류: {response.status}")
                    return files
                
                dir_contents = await response.json()
                api_call_counter['github'] += 1
                ...
                for item in dir_contents:
                    if item['type'] == 'file' and any(item['path'].endswith(ext) for ext in MAIN_EXTENSIONS):
                        ...
                        # 하위 디렉토리를 비동기로 처리
                        tasks.append(self.get_all_main_files_async(item['path'], session))
            ...
            print(f"[ERROR] 비동기 파일 목록 가져오기 실패: {e}")
        
        return files
    
    def get_all_main_files(self, path=""):
        """동기 wrapper 함수 (기존 코드 호환성 유지)"""
        return asyncio.run(self.get_all_main_files_async(path))
    ...
    async def get_file_contents_async(self) -> List[Dict[str, Any]]:
        """
        비동기로 파일 내용을 가져오는 개선된 함수
        """
        file_objs = []
        ...
        # aiohttp 세션 생성
        async with aiohttp.ClientSession() as session:
            # GitHub API 큐와 처리 큐 분리
            github_queue = Queue(maxsize=100)
            
            # 파일 경로를 큐에 추가
            for path in filtered_paths:
                await github_queue.put(path)
            
            # Worker 함수
            async def fetch_file_content(session, queue):
                results = []
                            ...
                            if response.status == 200:
                                ...
                                try:
                                    ...
                                    results.append({
                                        ...
                                        'content': content,
                                        'file_name': content_data['name'],
                                        'file_type': content_data['name'].split('.')[-1] if '.' in content_data['name'] else '',
                                        'sha': content_data['sha'],
                                        'source_url': content_data['html_url'],
                                    })
                                except Exception as e:
                                    print(f"[WARNING] 파일 디코딩 실패 {path}: {e}")
                    except Exception as e:
                        print(f"[ERROR] 파일 가져오기 실패: {e}")
                
                return results
            
            # 병렬 워커 실행 (50개 동시)
            workers = []
            for _ in range(min(50, len(filtered_paths))):
                workers.append(fetch_file_content(session, github_queue))
            
            results = await asyncio.gather(*workers)
            
            # 결과 통합
            for worker_results in results:
                file_objs.extend(worker_results)
        
        return file_objs
    
    def get_file_contents(self) -> List[Dict[str, Any]]:
        """동기 wrapper 함수 (기존 코드 호환성 유지)"""
        return asyncio.run(self.get_file_contents_async())
...
class RepositoryEmbedder:
    ...
    def process_and_embed(self, files: List[Dict[str, Any]]):
        ...
        async def async_process_and_embed(files):
            ...
            # 3. 비동기 병렬 실행 (동시 실행 수 증가: 50-100)
            print(f"[DEBUG] 임베딩+역할태깅 asyncio 병렬 처리 시작 (청크 수: {len(all_chunks)})")
            
            # OpenAI API 큐와 처리 큐 분리
            openai_queue = Queue()
            for chunk_data in all_chunks:
                await openai_queue.put(chunk_data)
            
            async with AsyncOpenAI(api_key=api_key) as client:
                # Semaphore를 100으로 증가 (API 제한에 따라 조정 가능)
                semaphore = asyncio.Semaphore(100)
                
                async def process_with_queue(queue):
                    results = []
                    while not queue.empty():
                        try:
                            args = await queue.get()
                            async with semaphore:
                                result = await embed_and_tag_async(args, client)
                                results.append(result)
                        except Exception as e:
                            print(f"[WARNING] 청크 처리 실패: {e}")
                    return results
                
                # 워커 수를 CPU 코어 수에 맞춰 조정
                import multiprocessing
                num_workers = min(multiprocessing.cpu_count() * 2, 50)
                
                workers = []
                for _ in range(num_workers):
                    workers.append(process_with_queue(openai_queue))
            ...
            print(f"[DEBUG] 임베딩+역할태깅 asyncio 병렬 처리 완료 (처리된 청크: {len(results)})")
            # 4. DB 저장 (동기)
            successful_saves = 0
